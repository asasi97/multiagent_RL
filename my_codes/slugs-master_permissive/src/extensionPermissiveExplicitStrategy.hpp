 ...
 */
template<class T, bool oneStepRecovery, bool jsonOutput> class XExtractPermissiveExplicitStrategy : public T {
protected:
    // New variables
    ...
    XExtractPermissiveExplicitStrategy<T,oneStepRecovery,jsonOutput>(std::list<std::string> &filenames): T(filenames) {}
...
public:
    ...
    void computeAndPrintExplicitStateStrategy(std::ostream &outputStream) {        
        ...
        while (todoList.size()>0) {
            ...
            while (!(remainingTransitions.isFalse())) {
                ...
                // Print
                outputStream << tn;
                if (!(remainingTransitions.isFalse())) {
                    outputStream << ", ";
                }
            }
            if (jsonOutput) {
    ...
    static GR1Context* makeInstance(std::list<std::string> &filenames) {
        return new XExtractPermissiveExplicitStrategy<T,oneStepRecovery,jsonOutput>(filenames);
    }
};
