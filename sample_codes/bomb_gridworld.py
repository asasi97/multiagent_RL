...
class GridWorld:
	...
	def __init__(self):
		...
		self.current_location = ( 4, np.random.randint(0,5)) # can possibly have a set starting positon
		self.current_location = (4,0)
...
class Q_Agent():
	...
	def choose_action(self, available_actions):
		...
		if np.random.uniform(0,1) < self.epsilon:
			...
			maxValue = max(q_values_of_state.values()) # finding the maximum q_value
...
def play(environment, agent, trials=500, max_steps_per_episode=1000, learn=False):
	...
	for trial in range(trials): # Run trials
		...
		while step < max_steps_per_episode and game_over != True: # Run until max steps or until game is finished
			...
			# can directly check if it's in a terminal state instead of checking with a function
...
# reward = env.make_step(chosen_action) # Similar to step
